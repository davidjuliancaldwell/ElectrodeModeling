%   SYNTAX 
%   examples_nastran_write_precise
%   DESCRIPTION 
%   The NASTRAN format is the oldest ASCII format for saving triangular
%   meshes. The present script converts P, t mesh data from MATLAB to a
%   NASTRAN file. If the P data is given in mm, the accuracy will be 1
%   micrometer
%
...
for m = 1:size(P, 1)
    ...
    num2str(P(m, 1), '%6.3f'), num2str(P(m, 2), '%6.3f'), num2str(P(m, 3), '%6.3f'));
end
for m = 1:size(t, 1)
