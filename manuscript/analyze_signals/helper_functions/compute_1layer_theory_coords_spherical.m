function [thy1arc,arcCorrection,thy1,correctionFactor,thy1scaled,correctionFactorScaled] = compute_1layer_theory_coords_spherical(locs,stimChans,rFixed)
...
arcCorrection = zeros(sizeData,1);
orrectionFactorScaled= zeros(sizeData,1);
...
for j=1:sizeData
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % arc length using bis
    
    bisbeta=dm/rFixed;
    bistop=dm+rFixed*(1-cos(bisbeta));
    bistheta=dp/rFixed;
    bisbottom=dp+rFixed*(1-cos(bistheta));
    arcCorrection(j)=(1/rFixed)*(log(bistop/bisbottom));
    correctionFactorTopBottomArc(j) = log(bistop)-log(bisbottom);
    
    thy1arc(j)=scale*((2/dp)-(2/dm)+(1/rFixed)*log(bistop/bisbottom));
    
end

thy1 = real(thy1);
thy1scaled = real(thy1scaled);
thy1arc = real(thy1arc);
end